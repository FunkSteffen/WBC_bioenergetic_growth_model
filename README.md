...
|:--:| 
|**Set-up of the individual-based bioenergetic growth model.** Left panel: Schematic representation of important model components and their influence on growth: (1) temperature conditions (SST and TDiff, see Material and methods) (2) temperature at residence depth, (3) food availability and intake, (4) physiological processes, and (5) growth in length. Right panel: Functional pathways (arrows) within the bioenergetic model: rectangles denote temperature variables (SST and TDiff), diamonds denote estimates derived from sub-models I-III; grey ellipses indicate estimates based on published physiological functions; hexagons display cod individual length and weight. i – time step, j – size, k – diet cluster.|
